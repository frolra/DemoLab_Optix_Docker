...
- [ ] After signing in as `webadmin` (or another `ldap-admins` member), generating a CSR immediately produces a signed certificate: verify the CSR itself is signature-valid (`step certificate inspect` or `openssl req -verify -noout`) with Subject/SANs matching the form input, the signed certificate's Subject/SANs match the CSR (confirming the x509 template fix in `install.sh` is in effect), the `.der` download is valid binary DER (`openssl x509 -inform der -in cert.der -noout -subject`), the `.pfx` download opens with the passphrase entered on the form (or with no passphrase if left blank) and contains the certificate, private key, and one intermediate certificate, and the `/generate` response has `Cache-Control: no-store`
- [ ] Deliberately break automatic signing (e.g. temporarily set the wrong `STEPCA_PASSWORD` for a test container) and confirm the result page still shows the CSR and private key with a clear inline error, instead of a blank page or a 500
