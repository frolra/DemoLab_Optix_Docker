...
- [ ] Re-run `./install.sh` a second time against an already-completed install (no destructive rebuild first): confirm it logs `ACME provisioner already present; skipping` at step `[2/12]`, `LDAP certificate already present; skipping regeneration` at step `[6/12]`, and `LDAP already bootstrapped (RootDN/webadmin); skipping` at step `[8/12]` instead of failing; confirm it completes all 12 steps, every already-running container is left healthy and undisturbed (`docker compose ps` shows no unexpected restarts of `step-ca`, `openldap`, `postgres`, `mysql`, `mssql`, or `mosquitto`), and `openldap`'s TLS listener still accepts connections afterward (`docker exec openldap ldapsearch -x -H ldaps://localhost:636 ...` or equivalent still succeeds — see [Troubleshooting](TROUBLESHOOTING.md) for the TLS-breakage bug this specifically guards against)
...
- [ ] CSR Generator at `https://csr.<HOSTNAME_FQDN>` redirects to `/login` when not authenticated, and rejects both a wrong password and a valid LDAP account that is *not* a member of `cn=ldap-admins,ou=Groups,dc=<HOSTNAME_FQDN>` (generic "Invalid credentials." in both cases, no distinction shown)
- [ ] After signing in as `webadmin` (or another `ldap-admins` member), clicking **Generate Key and Certificate** immediately produces a signed certificate: verify the CSR itself was signature-valid, the signed certificate's Subject/SANs match the form input (confirming the x509 template fix in `install.sh` is in effect), the `.der` download is valid binary DER (`openssl x509 -inform der -in cert.der -noout -subject`), the `.pfx` download opens with the passphrase entered on the form (or with no passphrase if left blank) and contains the certificate, private key, and one intermediate certificate, the CSR itself is not shown on this result page, and the `/generate` response has `Cache-Control: no-store`
- [ ] Clicking **Generate Key and Request (for external CA)** instead produces only a CSR (signature-valid - verify with `step certificate inspect` or `openssl req -verify -noout`) and a matching, parseable private key, with no signed certificate shown and no error banner (this is the intentional, unsigned path)
- [ ] Deliberately break automatic signing (e.g. temporarily set the wrong `STEPCA_PASSWORD` for a test container) and confirm clicking **Generate Key and Certificate** still shows the CSR and private key with a clear inline error, instead of a blank page or a 500
- [ ] No text/label overlaps any button on the CSR Generator's result page at default browser zoom (visually confirm the hint text under each card's action buttons has clear spacing, not overlapping)
