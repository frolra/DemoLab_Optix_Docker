...
- [ ] Re-run `./install.sh` a second time against an already-completed install (no destructive rebuild first): confirm it logs `ACME provisioner already present; skipping` at step `[2/12]` instead of failing, completes all 12 steps, and every already-running container is left healthy and undisturbed (`docker compose ps` shows no unexpected restarts of `step-ca`, `openldap`, `postgres`, `mysql`, `mssql`, or `mosquitto`)
