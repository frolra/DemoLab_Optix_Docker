...
The CSR Generator (`csr-generator`) also needs no bootstrap step: Docker Compose builds it from `./csr-generator` like `stepca-web` and brings it up directly. Unlike the original, anonymous version of this tool, it now reuses `settings.json`'s `ca.*`/`ldap.*` fields (mounted read-only, same file as `stepca-web`) for its CA/LDAP connection details, and needs `STEPCA_PASSWORD` (already required for step-ca itself) plus one new required `.env` variable of its own, `CSR_GENERATOR_SECRET_KEY` (signs its login session cookie, same minimum length as `STEPCA_WEB_SECRET_KEY`) — see [docs/CSR_GENERATOR.md](CSR_GENERATOR.md).
